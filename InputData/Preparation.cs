...
namespace InputData
{
    public class Sudoku
    {
        public Field[,] Preparation(BlockSquareMap blockSquareMap, OneFourSevenMap oneFourSevenMap, ref int globalcnt)
        {
            ...
            for (int i=0; i<rawLines.Length; i++)
                ...
                {
                    fields[i+1, j+1] = new Field(i+1, j+1, blockSquareMap, oneFourSevenMap[(i+1)*10 + j + 1]);
                    if (rawLines[i][j] != '.')
                    {
